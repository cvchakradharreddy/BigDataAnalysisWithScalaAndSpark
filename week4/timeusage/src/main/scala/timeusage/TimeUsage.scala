...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val lines = if (line.isEmpty) line else line.head +: line.tail.map(_.toDouble)
    Row.fromSeq(lines)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryList = List("t01", "t03", "t11", "t1801", "t1803")
    val workingList = List("t05", "t1805")
    val otherList = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val otherListExclude = List("t1801", "t1803", "t1805")
    (columnNames.filter(columnName => primaryList.exists(columnName.startsWith)).map(col),
      columnNames.filter(columnName => workingList.exists(columnName.startsWith)).map(col),
      columnNames.filter(columnName => otherList.exists(columnName.startsWith) && !otherListExclude.exists(columnName.startsWith)).map(col))
  }
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when(sex === 1, "male")
      .otherwise("female").as("sex")
    val ageProjection: Column = when(age >= 15 && age <= 22, "young")
      .when(age >= 23 && age <= 55, "active")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map(result => TimeUsageRow(result._1._1, result._1._2, result._1._3, result._2, result._3, result._4))
      .sort($"working", $"sex", $"age")
  }
}
